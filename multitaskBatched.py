...
x2data = torch.tensor(np.array(pd.read_csv('xdfi2.csv',index_col=0)))
y2data = torch.tensor(np.array(pd.read_csv('ydfi2.csv',index_col=0)))
xdat = np.array(xdata)
ydat = np.array(ydata)
...
train_n = len(train_x) 
testi2_x = scaler2.transform(x2data).double()
testi2_y = scaler3.transform(y2data).double()
...
means1 = torch.empty((1,3)).to(device)
...
with torch.no_grad():
    ...
    testi2_x = testi2_x.to(device)
    preds1 = likelihood(model(testi2_x))
    means1 = torch.cat([means1, preds1.mean])

means = means[1:]
means = means.to('cpu')
means1 = means1[1:]
means1 = means1.to('cpu')

testi2_y = testi2_y.to('cpu')
...
print(sklearn.metrics.mean_absolute_error(scaler3.inverse_transform_mean(testi2_y), scaler3.inverse_transform_mean(means1), multioutput='raw_values'))
print(sklearn.metrics.r2_score(scaler3.inverse_transform_mean(testi2_y), scaler3.inverse_transform_mean(means1)))
